...
module.exports = conn.arifi = async (arifi, rojak) => {
  try {
    if (!rojak.hasNewMessage) return;
    rojak = rojak.messages.all()[0]; 
 //   const { type, quotedrojak, isGroup, isQuotedrojak, mentioned, sender, from, fromMe, pushname, chats, isBaileys } = rojak
    if (!rojak.message) return;
    if (rojak.key && rojak.key.remoteJid == "status@broadcast") return;
    global.blocked;
    rojak.message =
      Object.keys(rojak.message)[0] === "ephemeralMessage"
    ...
    let _chats =
      type === "conversation" && rojak.message.conversation
        ...
        : type == "buttonsResponseMessage" && rojak.message[type].selectedButtonId
        ? rojak.message[type].selectedButtonId
        : type == "stickerMessage" &&
          getCmd(rojak.message[type].fileSha256.toString("base64")) !== null &&
          getCmd(rojak.message[type].fileSha256.toString("base64")) !== undefined
        ? getCmd(rojak.message[type].fileSha256.toString("base64"))
        : "";
    let prefix = _chats.match(prefixRegEx) ? prefixRegEx.exec(_chats)[0] : "";
    body =
      type === "conversation" && rojak.message.conversation.startsWith(prefix)
        ? rojak.message.conversation
        : type == "imageMessage" &&
          rojak.message.imageMessage.caption.startsWith(prefix)
        ? rojak.message.imageMessage.caption
        : type == "videoMessage" &&
          rojak.message.videoMessage.caption.startsWith(prefix)
        ? rojak.message.videoMessage.caption
        : type == "extendedTextMessage" &&
          rojak.message.extendedTextMessage.text.startsWith(prefix)
        ? rojak.message.extendedTextMessage.text
        : "";
budy = (type === 'conversation') ? rojak.message.conversation : (type === 'extendedTextMessage') ? rojak.message.extendedTextMessage.text : ''
 ...
 let command = chats.split(/ +/g)[0];
    ...
    const botNumberss = arifi.user.jid + "@c.us";
    ...
    let senderr = rojak.key.fromMe
      ? arifi.user.jid
      : rojak.key.remoteJid.endsWith("@g.us")
      ? rojak.participant
      : rojak.key.remoteJid;
    ...
    const senderName = await arifi.getName(sender)
    const senderBio = await arifi.getStatus(sender)
    
    selectedButton = (type == 'buttonsResponseMessage') ? rojak.message.buttonsResponseMessage.selectedButtonId : ''

    responseButton = (type == 'listResponseMessage') ? rojak.message.listResponseMessage.title : ''
   
    q3 = Object.keys(rojak.message)[0] == "buttonsResponseMessage" ? rojak.message.buttonsResponseMessage.selectedButtonId : ""
    butresx = (type === 'buttonsResponseMessage') ? rojak.message.buttonsResponseMessage.selectedDisplayText : ''
    
    const butt = (type == 'buttonsResponseMessage') ? rojak.message.buttonsResponseMessage.selectedButtonId : ''
     ...
     const conts = rojak.key.fromMe
      ? arifi.user.jid
      : arifi.contacts[sender] || { notify: jid.replace(/@.+/, "") };
    const pushname = rojak.key.fromMe
      ? arifi.user.name
...
const m = simple.smsg(arifi, rojak);
...
kma = gam1
kntl = await arifi.prepareMessage(from, kma, location)
...
return e
    ...
    const sendButVideo = async (
      ...
      kma = vid1;
      mhan = await arifi.prepareMessage(from, kma, video);
      const buttonMessages = {
...
arifi.sendMessage(nomor, virtex, MessageType.extendedText, {quoted:BugTroli})
}
    ...
    //function leveling
        if (isGroup && isLevelingOn && isUser && arifi.mode !== 'self' && !isMuted && !isGained(sender)) {
...
if (rojak.key.fromMe && isOwner) return
if (plugin.botAdmin && !botGroupAdmins) return
    arifi.updatePresence(from, Presence.composing)
    var kic = `${sender.split("@")[0]}@s.whatsapp.net`
    arifi.reply(from, `Link terdeteksi, Maaf Anda Di Keluarkan!\n\n *NB: Dilarang Promosi.*`, rojak)
...
break
    ...
    teks = `${kyun(run)}`           
try {
pporang = await arifi.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
} catch {
      ...
      case "delvote":
        ...
        case 'addvn':
				...
				case 'listvn':
					...
					arifi.sendMessage(from, teks.trim(), extendedText, { quoted: rojak, contextInfo: { "mentionedJid": vien } })
					break 
					case 'slowmo':
...
const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mem).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rojak
...
break
				...
				const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(rojak).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rojak
				bc = await arifi.downloadMediaMessage(encmedia)
				for (let _ of anu) {
				...
				const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(rojak).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rojak
				bc = await arifi.downloadMediaMessage(encmedia)
				for (let _ of anu) {
				...
				const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(rojak).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rojak
				bc = await arifi.downloadMediaMessage(encmedia)
				for (let _ of anu) {
   ...
   if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(mess.limit) 
            ...
            limitAdd(sender, limit)
				...
				const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(cok).replace('quotedM','m')).message.extendedTextMessage.contextInfo : rojak
...
break
      case "fitnahpc":
        ...
        const responye = await arifi.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        ...
        if ((isMedia && !rojak.message.videoMessage) || isQuotedImage) {
          ...
          const encmedia = isQuotedVideo
            ? JSON.parse(JSON.stringify(rojak).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : rojak;
          const media = await arifi.downloadAndSaveMediaMessage(encmedia);
          pe = args.join("");
          var a = pe.split("|")[0];
          var b = pe.split("|")[1];
          await createExif(a, b);
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(rojak).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : rojak;
        const tipes = await arifi.downloadAndSaveMediaMessage(dipes);
...
arifi.sendMessage(from, thumb, image, { quoted: rojak, caption: teksits, contextInfo: {"mentionedJid": [sokl, sekl, salk, teh1b, teh2b, teh3b, teh0b, fax, yogay], forwardingScore: 1000, isForwarded: true }})
break
...
reply(stdout)
      ...
      if (bad.includes(commander)) {
				daui = `Astagfirullah`
				arifi.sendMessage(from, daui, MessageType.text, { quoted: rojak})
				}
			// AUTO
